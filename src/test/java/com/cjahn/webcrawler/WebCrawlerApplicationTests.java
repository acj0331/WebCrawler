...
public class WebCrawlerApplicationTests {
	...
	public void esTest() throws UnsupportedEncodingException {
		...
		item.setDescription(
				"Nacel International School System- NISS 1964년 미국 뉴욕주에 설립된 비영리교육재단인 Nacel재단에 의해 개설된 Nacel International School System 9개국 15개 도시 19개 학교를 운영하고 있답니다. NISS... ");
...
//		System.out.println(itemList.size());
	...
	public void elasticsearchQueryTestCount() {
		SearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(matchAllQuery()).withIndices("web_items")
				.withTypes("web_item").build();
	...
	public void elasticsearchQueryList() {
		SearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(matchAllQuery()).withIndices("web_items")
				.withTypes("web_item").withSort(new FieldSortBuilder("id").order(SortOrder.DESC)).build();
	...
	public void elasticsearchQueryList2() {
		...
		for (int i = 0; i <= maxPage; i++) {
			searchQuery = new NativeSearchQueryBuilder().withIndices("web_items").withTypes("web_item")
					.withQuery(matchAllQuery()).withSort(new FieldSortBuilder("id").order(SortOrder.DESC))
	...
	public void elasticsearchQueryList3() {
		SearchQuery searchQuery = new NativeSearchQueryBuilder().withIndices("web_items").withTypes("web_item")
//			      .withQuery(matchAllQuery())
//			      .withSort(new FieldSortBuilder("id").order(SortOrder.DESC))
				.withQuery(matchQuery("base64",
						"aHR0cHM6Ly9ibG9nLm5hdmVyLmNvbS9odWFuaT9SZWRpcmVjdD1Mb2cmYW1wO2xvZ05vPTIyMTUwMTI4NjMzNg"))
	...
	public void seleniumTest() {
		...
		driver.get("http://www.kns.tv/news/articleView.html?idxno=555581");
